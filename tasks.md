...
|---|---|---|---|
| **M1 Platform & Data** | **Rishit (@rizzit17)** | monorepo, CI, `services/api` shell, DB + Alembic, EventBus, WS gateway, event store, `cabos_core.mapping`, import pipeline, auth/RBAC | `/studio` (Data Studio), `/command/*` console shell, `/director` UI, `/trainer`, instructor booking, coaching cards |
| **M2 Intelligence (ML)** | **Aditya (@Adi07289)**: also tech lead | `cabos_core.intervals`, `hypotheses`, `pulse`, `forecast`, synthetic generator, model registry, Copilot backend + `LLMProvider` | `/cab/me` (My Pulse), task detail + factor waterfall, `/command/model`, `/command/plan`, `/command/anomalies`, hypothesis cards |
| **M3 Safety & Simulation** | **Sid (@sidverma2407-png)** | `services/edge-sim`, `cabos_core.safety` (seatbelt, proximity, conditions, fatigue), weather service, incidents backend, Director scenario handlers | `/cab/guardian` (radar, belt panel), alert banner + safety overlay, near-miss sheet, `/command/incidents`, swing simulator |
| **M4 Experience & Design** | **Priyal (@priyal2905)** | design tokens, themes, primitives, fonts, sound/haptics kit, i18n, offline outbox (IndexedDB), realtime client store | `/design`, `/onboarding/*`, `/cab` Shift Deck + CabShell, `/cab/academy/*`, landing `/`, `/cab/debrief` |
...
**Legend used in every table below:** M1 = Rishit (@rizzit17) · M2 = Aditya (@Adi07289) · M3 = Sid (@sidverma2407-png) · M4 = Priyal (@priyal2905)

### Tech lead duties: Aditya (@Adi07289)
...
- **M1 ↔ M4** (Rishit (@rizzit17) ↔ Priyal (@priyal2905)): API, contracts and web wiring.
- **M2 ↔ M3** (Aditya (@Adi07289) ↔ Sid (@sidverma2407-png)): `cabos-core` Python, ML and safety logic.
...
|---|---|---|
| M1 Rishit (@rizzit17) | 22 + 1 shared | P2 (data spine critical path) |
| M2 Aditya (@Adi07289) | 20 + tech lead | P5 (Pulse + Forecast) |
| M3 Sid (@sidverma2407-png) | 16 + 1 shared (fewer, but the heaviest: radar, state machines, simulator) | P4 (Guardian) |
| M4 Priyal (@priyal2905) | 17 + 2 shared | P1 + P3 (design system, onboarding, deck) |
...
|---|---|---|
| **M1 Rishit (@rizzit17)** | P1-01 monorepo + Makefile → P1-02 CI → P1-03 FastAPI shell | Yes: this unblocks everyone, so do it first |
| **M2 Aditya (@Adi07289)** | P1-04 `cabos-core` + Clock → P1-10 config drafts → P2-03 intervals (fixture test 0.705 / 1.90 / gap) | Yes: pure Python |
| **M3 Sid (@sidverma2407-png)** | P1-09 edge-sim skeleton + `safety.yaml` → P4-01 seatbelt state machine (pure Python, table-driven test) early | Yes: the seatbelt SM needs only `cabos-core` |
| **M4 Priyal (@priyal2905)** | P1-05 Next.js + tokens → P1-06 themes → P1-07 primitives → P1-08 `/design` | Yes, once P1-01 lands (same day) |
